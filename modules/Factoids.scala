...
class Factoids(ctl: Control) extends Module(ctl) with Auth with Commands {
    ...
    def sendFact(to: String, pattern: String, sendError: Boolean) = lookup(pattern) match {
    ...
    def searchFacts(from: Prefix, to: String, pattern: String) = {
        try {
